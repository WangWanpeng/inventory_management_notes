...
\begin{equation*}
  \theta = \EE{X} = \E(N(L))\E(D_i) = \lambda L d=\frac{T}{\E(T)} d,
\end{equation*}
...
&= \lambda l \EE{D^2}, \quad\text{since in general } \V(X) = \EE{X^2} - (\E(X))^2.
  \end{split}
\end{equation*}
Here
\begin{equation*}
  \EE{D^2} = \sum_{i=1}^\infty i^2\P(D=i).
\end{equation*}
...
\end{comment}
%\subsection{Auto-Correlated Demand}
...
\begin{landscape}
\let\thefootnote\relax\footnotetext{\textbf{Exercise~\ref{plots}}}
\begin{center}
...
\begin{landscape}
\let\thefootnote\relax\footnotetext{\textbf{Exercise~\ref{inter}}}
