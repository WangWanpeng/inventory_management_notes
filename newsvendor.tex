...
% Once again, the problem is to determin the optimal order size $Q^*$ that maximizes the revenue. \footnote{We use the notation of Factory Physics (FP). Note that the formulas in
...
\begin{exercise}[Continuation]  What is the cost if $Q=1$?  Hence, what is the optimal choice for $Q$?
\begin{solution}
\begin{align*}
&h\E{(1-D)^+} + h\E{(1-D)^-} \\
...
\begin{exercise}[Continuation of Ex.~\ref{ex:10} and \ref{ex:55}]
...
%  sell the minimum of $$ and $W$ units at a price of $s$, and we will not be able to salvage the rest. Therefore we will lose $s\ex \left((Q-D)^+ - \min\{(Q-D)^+,W\}\right)$
...
% That's why the we have $L(0)\approx 10\cdot20=200$. This is the very same cost that would realize if demand was exactly 20 units and the order quantity was 0.
...
% \begin{solution}
%   First the data. We also use a numerical library (scipy.stats) to handle the computations for
...
%     $\P{D<0}=\Phi((D-\mu)/\sigma) < 0$ is not small anymore. However,
...
% \begin{remark}[\rocket] Sometimes the demand is modeled as a continuous random variable, in which case the cost  $L(Q)$ takes  the form
% \begin{align*}
% L(Q) & = h \E{(Q-D)^+} + b \E{(Q-D)^-} \\
%      & = \int_0^\infty \left(h (Q-x)^+ + b (Q-x)^-\right) f(x) \d x
% \end{align*}
% where $f(x)$ is the \emph{probability density function} of $D$  and has the conceptual meaning
% of $f(x)\d x=\P{X\in[x, x+\d x]}$. For most practical cases, this formula is less useful than~\eqref{eq:59}, simply because the integrals have to be carried out numerically. Nearly only  the demand is normally distributed the above is useful.  For this reason, we do not discuss this further.
% \end{remark}
...
\begin{solution}
See Figure~\ref{fig:maxV}. \nvf{todo} We have that $V^\#(x)=cx+V(Q^*)$ for all $x<Q^*$ and $V^\#(x)=cx+V(x)$ for all $x\geq Q^*$.
% \begin{figure}[htbp]
% \centering
% \begin{tikzpicture}
% \scriptsize
...
The discussion above leads to the following characterization of the optimal order quantity. Let $Q^*$ be the optimal order quantity in the original newsvendor case with no initial inventories. The previous exercise explains that, when starting with $x<Q^*$, we still order up to $Q^*$. Thus, 
the optimal policy of newsvendor is a so-called \emph{base-stock policy}:  it is optimal to `order up-to' the \emph{base stock level} $Q^*$ whenever the inventory level is below this level and not to order otherwise. As a consequence,  it is actually better to interpret the newsvendor policy as an `order up-to' policy than an `order quantity' policy. Practically they may seem to boil down to the same thing, but the former interpretation turns out to be more intuitive and practical since the optimal order up-to level is independent from the current inventory level whereas the order quantity most certainly is not.


It is because the optimal ordering policy is a base-stock policy. Following a base-stock policy, if what we have on-hand is smaller than the base-stock level, we order up-to the base-stock level, and otherwise we do not order. Therefore, what we order is strictly linked to what we already have. 
\end{comment}
...
\nvf{Here we see that it it much easier to think in terms of order-up-to levels, as they remain constant, than in terms of order quantities, the latter depends on the order-up-to level and the starting inventory level.}

% \begin{exercise}
% Why is the problem for the  two-period newsvendor model essentially different from two independent newsvendor models (i.e., a newsvendor problem for the morning and another for the afternoon)?
% \begin{solution}
% The two-period newsvendor model has two ordering moments, and the ordering decision made in the first stage has consequences for the second stage. The dynamics of the problem are as follows. Let's assume we order $Q_1$ units in the first period. The leftover inventory at the end of the first period will then be $(Q_1-D_1)^+$. We know from Section~\ref{sec:newsvendor_starting} that we would like to order up to a certain level. Hence, the  ordering quantity for the afternoon should depend on what remains from the morning, i.e., on $(Q_1-D_1)^+$.

% For the second period: what has happened (i.e. shortages, costs, etc.) in the first period is no longer of our concern when we deal with the second period. This means that we can think of the second period problem completely independent from the first, apart from $(Q_1-D_1)^+$. .
% \end{solution}
% \end{exercise}
...
\begin{exercise}
Given the starting inventory level $x$, show that optimal profit \nvf{Make a $V$ for a one period problem} for the second period is
...
% 	axis lines=middle,
% 	ymin=0]
% \addplot+[no markers] table [y=V1, x=Q1]{data.dat};
% \addlegendentry{$Z_1(Q_1)$}
% \end{axis}
% \end{tikzpicture}
% \caption{First stage expected profit function}
% \label{fig:V1}
% \end{figure}
