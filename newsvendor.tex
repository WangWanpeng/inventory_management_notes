...
\begin{comment}
 sell the minimum of $\Q$ and $W$ units at a price of $s$, and we will not be able to salvage the rest. Therefore we will lose $s\E{\left((\Q-D)^+ - \min\{(\Q-D)^+,W\}\right)}$
...
The discussion above leads to the following characterization of the optimal order level. Let $\Q^*$ be the optimal order level in the original newsvendor case with no initial inventories. The previous exercise explains that, when starting with $x<\Q^*$, we still order up to $\Q^*$. Thus, 
the optimal policy of newsvendor is a so-called \emph{base-stock policy}:  it is optimal to `order up-to' the \emph{base stock level} $\Q^*$ whenever the inventory level is below this level and not to order otherwise. As a consequence,  it is actually better to interpret the newsvendor policy as an `order up-to' policy than an `order level' policy. Practically they may seem to boil down to the same thing, but the former interpretation turns out to be more intuitive and practical since the optimal order up-to level is independent from the current inventory level whereas the order level most certainly is not.
...
%\begin{solution}
The two-period newsvendor model has two ordering moments, and the ordering decision made in the first stage has consequences for the second stage. The dynamics of the problem are as follows. Let's assume we order $\Q_1$ units in the first period. The leftover inventory at the end of the first period will then be $(\Q_1-D_1)^+$. We know from Section~\ref{sec:newsvendor_starting} that we would like to order up to a certain level. Hence, the  ordering level for the afternoon should depend on what remains from the morning, i.e., on $(\Q_1-D_1)^+$.
